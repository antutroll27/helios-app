...
-- Run in Supabase SQL Editor after existing schema_v1
-- ================================================================
...
-- A1. users — add body metrics + wearable IDs + pharmacokinetic fields
...
-- A2. sleep_logs — add granular metrics missing from v1
...
ALTER TABLE public.sleep_logs
  ADD CONSTRAINT IF NOT EXISTS sleep_logs_user_date_unique UNIQUE (user_id, date);

-- A3. biometric_logs — expand metric enum and add sleep_log FK
...
-- A4. data_imports — add storage path and file size
...
-- pgvector embeddings upgrade path (Phase 2)
...
-- ================================================================
-- SECTION J: SUPABASE STORAGE BUCKET (manual step)
-- ================================================================
-- Create via Supabase Dashboard → Storage → New bucket, OR via API:
--
...
-- ================================================================
-- SECTION K: RESEARCH MODULE → TABLE MAPPING (reference only)
-- ================================================================
-- chronotype_engine.ChronotypeEngine   → reads sleep_logs (7-14 days)
-- chronotype_engine.ProtocolScorer     → reads protocol_logs JOIN sleep_logs
--                                      → writes protocol_effectiveness
-- space_weather_bio.SpaceWeatherBioModel → reads space_weather_cache
--                                        → writes daily_summaries.kp_max, disruption_score
-- caffeine_model.CaffeineModel         → reads caffeine_logs + users.cyp1a2_genotype
-- alcohol_model.AlcoholModel           → reads alcohol_logs JOIN sleep_logs
-- breathwork_model.BreathworkModel     → reads breathwork_sessions + biometric_logs
-- nap_model.NapModel                   → reads sleep_logs.wake_time, nap_logs
-- light_model.CircadianLightModel      → reads light_exposure_logs
