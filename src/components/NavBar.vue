...
<template>
  <nav class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 py-2.5 bg-(--glass-bg) backdrop-blur-[20px] border-b border-(--glass-border)">
    <!-- Left: Location -->
    <div class="flex items-center gap-1.5 min-w-0">
      <MapPin :size="13" class="text-(--text-muted) shrink-0" />
      <div class="flex flex-col">
        <span class="font-mono text-[0.65rem] font-semibold text-(--text-primary) tracking-[0.03em]">{{ geo.locationName }}</span>
        <span class="font-mono text-[0.45rem] text-(--text-muted) tracking-[0.05em]">{{ geo.lat.toFixed(2) }}°, {{ geo.lng.toFixed(2) }}°</span>
      </div>
      <span class="text-[0.4rem] text-[#00D4AA] ml-[0.15rem] pulse-live">●</span>
    </div>

    <!-- Center: Logo SVG -->
    <div class="absolute left-1/2 -translate-x-1/2 flex flex-col items-center overflow-visible py-1">
      <svg class="h-9 w-auto text-(--text-primary) block" viewBox="0 0 501 427" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <span class="font-mono text-[0.35rem] tracking-[0.25em] text-(--text-muted) mt-[0.1rem]">CIRCADIAN INTELLIGENCE</span>
    </div>

    <!-- Right: Controls -->
    <div class="flex items-center gap-[0.15rem]">
      <button class="p-1.5 rounded-md border-none bg-transparent cursor-pointer transition-[background] duration-150 flex items-center justify-center hover:bg-white/[0.08] disabled:opacity-40 disabled:cursor-wait" @click="fullReload" title="Restart demo" :disabled="isReloading">
        <RotateCcw :size="15" :class="{ 'spin-anim': isReloading }" class="text-(--text-secondary)" />
      </button>
      <button class="p-1.5 rounded-md border-none bg-transparent cursor-pointer transition-[background] duration-150 flex items-center justify-center hover:bg-white/[0.08]" @click="toggle" :title="isDark ? 'Light mode' : 'Dark mode'">
        <Sun v-if="isDark" :size="15" class="text-(--text-secondary)" />
        <Moon v-else :size="15" class="text-(--text-secondary)" />
      </button>
      <button class="p-1.5 rounded-md border-none bg-transparent cursor-pointer transition-[background] duration-150 flex items-center justify-center hover:bg-white/[0.08]" @click="router.push('/settings')" title="Settings">
        <Settings :size="15" class="text-(--text-secondary)" />
      </button>
    </div>
  </nav>
</template>
