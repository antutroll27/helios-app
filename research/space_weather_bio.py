...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("HELIOS Space Weather Biological Impact Report — G-Scale Staged Model")
    print(f"Timestamp: {reading.timestamp.strftime('%Y-%m-%d %H:%M UTC')}")
    print(
        f"Kp: {reading.kp_index}  |  "
        f"Solar Wind: {reading.solar_wind_speed} km/s  |  "
        f"Bz: {reading.bz} nT  |  "
        f"Latitude: {reading.latitude}°"
    )
    print("=" * 70)

    bz = model.bz_storm_predictor(reading.bz, reading.solar_wind_speed)
    print(f"\n--- Bz Storm Predictor (physics layer) ---")
    print(f"Storm imminent: {bz['storm_imminent']} | Coupling: {bz['southward_coupling']}")
    print(f"Propagation time: ~{bz['propagation_min']} min")
    print(f"Advisory: {bz['advisory']}")

    hrv = model.kp_hrv_impact(reading.kp_index, latitude=reading.latitude)
    print(f"\n--- HRV Impact ({hrv['g_scale']} — Alabdali 2022, n=809) ---")
    print(
        f"rMSSD delta: {hrv['delta_rmssd']} ms  |  "
        f"Suppression: {hrv['suppression_pct']}%  |  "
        f"Lat modifier: {hrv['latitude_modifier']}x"
    )
    print(f"Extrapolated: {hrv['is_extrapolated']} | Adaptive possible: {hrv['adaptive_response_possible']}")
    print(f"Advisory: {hrv['advisory']}")

    cog = model.kp_cognitive_advisory(reading.kp_index)
    print(f"\n--- Cognitive ({cog['g_scale']} — Liddie 2024, n=1,081) ---")
    print(
        f"Impact: {cog['impact_tier']}  |  "
        f"Focus modifier: {cog['focus_modifier']}  |  "
        f"Working memory: {cog['working_memory_impact']}"
    )
    print(f"Advisory: {cog['advisory']}")

    bp = model.bp_advisory(reading.kp_index, latitude=reading.latitude)
    print(f"\n--- BP Advisory ({bp['g_scale']} — Chen 2025, n=554k) ---")
    print(f"Systolic fluctuation: {bp['systolic_fluctuation_mmhg']} mmHg | Relevance: {bp['clinical_relevance']}")
    print(f"Advisory: {bp['advisory']}")

    comp = model.composite_disruption(reading)
    print(f"\n--- Composite ({comp['g_scale']} — G-Scale Staged) ---")
    print(f"Biological alert: {comp['biological_alert']}")
    print(
        f"HRV suppression: {comp['hrv_suppression_pct']}%  |  "
        f"Focus modifier: {comp['cognitive_focus_modifier']}  |  "
        f"BP: {comp['bp_systolic_fluctuation_mmhg']} mmHg"
    )
    print("Protocol adjustments:")
    for adj in comp["protocol_adjustments"]:
        print(f"  * {adj}")
    print(f"Mechanism: {comp['mechanism_note']}")
